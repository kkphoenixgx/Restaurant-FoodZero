...
export default class DaoPost {
  ...
  public async listPosts(limit?: number, offset?: number): Promise<Post[]> {
    let query = `SELECT p.id, p.created_at, p.description,
              u.id as user_id, u.name, u.email, u.password, u.user_image, u.phone, u.role
         FROM posts p
    LEFT JOIN users u ON p.user_id = u.id
  ...
  public async updatePost(id: number, newPost: Post): Promise<void> {
    ...
    try {
      ...
      for (const tagData of newPost.tags) {
        let tag = await this.daoTag.getTagByName(tagData.name);
        let tagId: number;
        if (!tag) {
          tagId = await this.daoTag.postTag(new Tag(tagData.name, 0));
        ...
        await this.daoPostTag.linkPostToTag(id, tagId);
      }
      await this.connection.commit();
    } catch (error) {
      await this.connection.rollback();
  ...
  public async deletePostById(id: number): Promise<void> {
    ...
    try {
      await this.daoPostTag.deleteTagsForPost(id);
      await this.connection.execute(`DELETE FROM comments WHERE post_id = ?`, [id]);
